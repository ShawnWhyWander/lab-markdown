...
for file in files:
    with open (file, encoding = 'utf8') as fin:
        text = fin.read()
        #print(text)
        data += json.loads(text) #data.append(json.loads(text)) will create lists in a list, so we use +=
        # we can also use data.extend(text), same effect as data +=
...
# print ('text', data[0]['text']) ## 0 is not inside 
...
for i, tweet in enumerate(data):
    #this is wrong:
    # if 'Trump' or 'trump' in tweet['text']:
    # because it is divided as if ('Trump') or '('trump' in tweet['text']):
    #correct version is:
